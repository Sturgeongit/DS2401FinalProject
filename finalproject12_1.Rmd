...
# Looking at our filtered data frame
```{r}
...
```
# Looking at data grouped by genre and setting columns to look at max runtime and avg runtime
```{r}
...
```
Here is where we noticed that we needed to clean up the genre catetory and set a "Main Genre" 

---

We need to categorize each complex sub-genre into one of nine major genres. We will start by using a for loop and "startswith"

```{r, echo=TRUE}

#splitting the genre column by ","
movies$subgenre <- str_split(movies$genre, "[,]", simplify = TRUE) [,1]

#now creating the 9 major genres
...
genre_only <- new_movies %>% 
  group_by(Genre, time_increment) %>% 
  summarise(count = n())
...
genre_only %>% 
  ggplot(aes(x = Genre, y = count , fill = as.factor(time_increment))) +
